        ...
        unsigned int port, char *mapped_res)
{
    ...
    //write to mapped_data
    write_record(pms->src_addr, pms->src_port, pms->dest_addr, pms->listen_port);
    return 0;
...
mapped_server_handler(int fd, int event, void *opaque)
{
    ...
    if (remote_fd < 0) {
        ...
        delete_record(pms->src_addr, pms->src_port);
        free(pms);
        return -1;
    }
...
recover_mapped_pair()
{
    char t_src_addr[INET_ADDRSTRLEN], t_mapped_addr[INET_ADDRSTRLEN];
    unsigned int t_src_port, t_mapped_port;
    
    int ret;
    struct mapped_server *pms;
    start_read();
    char t_not_response_addr[100][32];
    unsigned int t_not_response_port[100];
    int t_not_response_cnt = 0;
    while(read_record(t_src_addr, &t_src_port, t_mapped_addr, &t_mapped_port) != EOF) {
        //recover  
        int tmpfd = connect_to(t_src_addr, t_src_port); 
        if (tmpfd < 0) {
            strncpy(t_not_response_addr[t_not_response_cnt], t_src_addr, strlen(t_src_addr)+1);
            t_not_response_port[t_not_response_cnt] = t_src_port;
            ++t_not_response_cnt;
            continue;
        }
        else {
        ...
        ret = create_server(t_mapped_port);
        if (ret < 0) {
        
            LOG_ERR("recover server error.[t_src_addr:%s] [t_src_port:%u] [t_mapped_addr:%s] [t_mapped_port:%u]", 
                    t_src_addr, t_src_port, t_mapped_addr, t_mapped_port);
            continue;
        }
        g_pls->used_ports[t_mapped_port] = 1;
        LOG_INFO("recover server success.[t_src_addr:%s] [t_src_port:%u] [t_mapped_addr:%s] [t_mapped_port:%u]", 
                    t_src_addr, t_src_port, t_mapped_addr, t_mapped_port);


        pms = (struct mapped_server *)malloc(sizeof(struct mapped_server));
        g_pls->accepted_num += 1;
        pms->sockfd = ret;
        pms->accepted_num = 0;
        strncpy(pms->src_addr, t_src_addr, INET_ADDRSTRLEN);
        pms->src_port = t_src_port;
        pms->listen_port = t_mapped_port;
        pms->ls = g_pls;
        get_local_ip(pms->dest_addr);
    ...
    int i = 0;
    for (i = 0; i < t_not_response_cnt; ++i) {
        delete_record(t_not_response_addr[i], t_not_response_port[i]);
    }
    return 0;
}
